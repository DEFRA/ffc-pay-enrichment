...
async function processPaymentMessage (message, receiver) {
  ...
  try {
    ...
    await sendMessage({ paymentRequest, accepted: true }, ACCEPTED, { sourceSystem: paymentRequest.sourceSystem })
    ...
    if (err.category === VALIDATION) {
      await sendMessage({ paymentRequest, accepted: false, error: err.message }, REJECTED, { sourceSystem: paymentRequest?.sourceSystem })
