...
const processCustomerMessage = async (message, receiver) => {
  ...
  try {
    console.log(`Customer update received: vendor: ${update.vendor}, sbi: ${update?.sbi}, frn: ${update?.frn}`)
    if (
      !update?.frn ||
    ...
    console.error(`Unable to process customer update request: sbi: ${update?.sbi}, frn: ${update?.frn}`)
